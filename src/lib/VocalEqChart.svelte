<script>
  let { refCurve = null, mixCurve = null } = $props()
  ...
  function curveToArea(curve, closeSide) {
    if (!curve) return ''
...
<svg viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg" class="eq-svg">
  ...
  <!-- reference curve fill -->
  {#if refCurve}
    <polygon points={curveToArea(refCurve)} fill="rgba(201,168,76,0.06)" clip-path="url(#chart-clip)" />
    <polyline points={curveToPoints(refCurve)} fill="none" stroke="#c9a84c" stroke-width="1.5"
      stroke-linejoin="round" stroke-linecap="round" clip-path="url(#chart-clip)" />
  {/if}

  <!-- mix curve fill -->
  {#if mixCurve}
    <polygon points={curveToArea(mixCurve)} fill="rgba(74,159,212,0.06)" clip-path="url(#chart-clip)" />
    <polyline points={curveToPoints(mixCurve)} fill="none" stroke="#4a9fd4" stroke-width="1.5"
      stroke-linejoin="round" stroke-linecap="round" clip-path="url(#chart-clip)" />
  {/if}

  <!-- legend -->
  {#if refCurve}
    <line x1={PAD_L + 4} y1={PAD_T + 8} x2={PAD_L + 18} y2={PAD_T + 8} stroke="#c9a84c" stroke-width="1.5" />
    <text x={PAD_L + 22} y={PAD_T + 11.5} font-size="9" fill="#c9a84c" font-family="Space Mono, monospace">REFERENCE</text>
  {/if}
  {#if mixCurve}
    <line x1={PAD_L + 90} y1={PAD_T + 8} x2={PAD_L + 104} y2={PAD_T + 8} stroke="#4a9fd4" stroke-width="1.5" />
    <text x={PAD_L + 108} y={PAD_T + 11.5} font-size="9" fill="#4a9fd4" font-family="Space Mono, monospace">MY VOCAL</text>
  {/if}
</svg>
